...
주요 key와 로컬 경로는 env에서 관리한다. `phase0/.env.example`을 `phase0/.env`로 복사한 뒤 장비별 값을 수정한다. 실제 `.env`는 git에 올라가지 않는다.

주요 env key:

- `GLOSS_PHASE0_ACTIVE_MODEL_PROFILE`: 현재 모델 프로파일. 기본값 `qwen3-4b`
- `GLOSS_PHASE0_MODEL`: 런타임 모델명 직접 override. 비워두면 profile의 `runtime_model` 사용
- `GLOSS_PHASE0_BASE_URL`: OpenAI 호환 endpoint
...
- `GLOSS_PHASE0_OUTPUT_DIR`: 측정 산출물 저장 위치
